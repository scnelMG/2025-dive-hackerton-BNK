...
발표자료의 현황 분석에서는 비이자상품 미이용 고객 비중을 **63%**로 제시했습니다. 이 수치는 [실제 최종 발표자료](docs/IBA_서호영.pptx)의 현황 분석을 근거로 하며, 원본 고객 데이터는 공개하지 않습니다. 비이자이익은 단순히 고기여 고객을 찾는 문제가 아니라, 기존 고객의 거래 관계를 어떻게 확장할지에 대한 문제로 보았습니다.
...
비이자이익 군집은 단순 우열이 아니라 `Basic → Standard → Premium → VIP`의 고객 성장 경로로 해석했습니다. 최종 발표자료 기준 각 군집은 Basic **1,428,342명(68.0%)**, Standard **289,320명(13.8%)**, Premium **268,646명(12.8%)**, VIP **107,219명(5.1%)**으로 정리했습니다.
...
저는 팀에서 **구간형 데이터를 분석 가능한 수치 데이터로 변환하고, 정제·군집화 모델을 구축**했습니다.

- 구간형 금융 데이터를 수치형 대표값으로 변환
...
2. [결과 및 전략](docs/results.md) — 군집·상품별 해석과 제안
3. [최종 발표자료](docs/IBA_서호영.pptx) — 원본 최종 발표 산출물
4. [발표 이미지 출처](docs/presentation-assets.md) — README에 사용한 실제 슬라이드의 원본 위치
5. [노트북 안내](notebooks/README.md) — 코드 흐름과 실행 전제
